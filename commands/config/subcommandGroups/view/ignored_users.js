...
module.exports = async (interaction) => {
	...
	if (guildConfig && guildConfig.ignoredUsers && guildConfig.ignoredUsers.length > 0) {
		const ignoredUsers = guildConfig.ignoredUsers.join(', ');
		interaction.editReply(ignoredUsers);
	} else {
		interaction.editReply('This server does not have any ignored users set.');
	}
};