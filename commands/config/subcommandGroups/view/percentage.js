...
module.exports = async (interaction) => {
	...
	if (guildConfig && guildConfig.percentage) {
		interaction.editReply('`' + guildConfig.percentage + '%`');
	} else {
		interaction.editReply('This server does not have a value set for percentage. Without this value set, it will default to `70%`');
	}
};